...
if(Path::is_post_request())
{
    ...
    $dPass = $row['password'];
    if (password_verify($old_pass, $dPass)){
        if($new_pass == $con_pass){
            $password = password_hash($new_pass, PASSWORD_BCRYPT);
            $stmt = $connection->prepare("UPDATE users SET password='$password', updationDate='$currentTime' WHERE id='$user_id'");
            if ($stmt->execute()){
							...
							<div class="row">
								...
								<p style="color:red;"><?php echo Error::customized_display_error($error);?>
                                    <?php echo Notification::display_message(); ?></p>
													<form role="form" action ="<?php echo Path::url_for('change-password.php'); ?>" name="chngpwd" method="post" onSubmit="return valid();">
