...
if(Path::is_post_request()){
    ...
    $found_account = Patient::use_for_ip($email);
    if(empty($errors)){
        $patient = Patient::find_by_email($email);

    }

    if ($found_account && $patient->verify_password($password)){
        $user_session->store($patient);
        $mail_to_get_id = $found_account['id'];
        $email_itself = $found_account['email'];
        $host = $_SERVER['HTTP_HOST'];
        $uip = $_SERVER['REMOTE_ADDR'];
        $status = 1;
        $user_log = new Userlog($mail_to_get_id, $email_itself, $uip, $status);
        $user_log->create();
        Path::redirect_to(Path::url_for('dashboard.php?patient_id=' . $mail_to_get_id));
    }else{
        $mail_to_get_id = 'NULL';
        $email_itself = $email;
        $uip = $_SERVER['REMOTE_ADDR'];
